...
class REI1Tokenizer:
  ...
  def _setup_token_patterns(self):
    ...
    self.identifier_pattern = re.compile(
        r'[a-zA-Z_α-ωΑ-Ω∀-∿][a-zA-Z0-9_α-ωΑ-Ω∀-∿\'-]*(?:\.[a-zA-Z_α-ωΑ-Ω∀-∿][a-zA-Z0-9_α-ωΑ-Ω∀-∿\'-]*)*')
  ...
  def _match_token_at_position(self, line: str, pos: int, line_num: int) -> Optional[Token]:
    ...
    if line[pos] == '"':
      ...
      if string_match:
        ...
        span = SourceSpan(
            self.filename, line_num, pos +
            1, line_num, pos + len(value) + 1, value
        )
...
class REI1Grammar:
  ...
  def _setup_grammar(self):
    ...
    string_literal = QuotedString(
        '"', escChar='\\').setParseAction(lambda t: ("STRING", t[0]))
    char_literal = Regex(r'`(?:[^`\\]|\\.)').setParseAction(
        lambda t: ("CHAR", t[0][1:]))
    number = Regex(r'-?(?:0|[1-9]\d*)(?:\.\d+)?').setParseAction(
        lambda t: ("NUMBER", float(str(t[0]))
                   if '.' in str(t[0]) else int(str(t[0])))
    )
    # Note: True and False are now constructors, not boolean literals

    # Identifiers (simplified) - base pattern
    identifier_base = Regex(
        r'[a-zA-Z_][a-zA-Z0-9_\'-]*(?:\.[a-zA-Z_][a-zA-Z0-9_\'-]*)*')

    # Constructor identifier (starts with uppercase)
    constructor_identifier = Regex(
        r'[A-Z][a-zA-Z0-9_\'-]*(?:\.[A-Z][a-zA-Z0-9_\'-]*)*')

    # Keywords to exclude from identifiers
    excluded_keywords = (case_kw | of_kw | lambda_kw |
                         where_kw | pre_kw | post_kw | get_kw)

    # Context-specific identifiers with keyword exclusion
    value_identifier = (~excluded_keywords + identifier_base.copy()
                        ).setParseAction(lambda t: ("IDENTIFIER", t[0]))
    binding_identifier = ~excluded_keywords + \
        identifier_base.copy()  # Raw string for binding names
    pattern_identifier = ~excluded_keywords + \
        identifier_base.copy()  # Raw string for pattern matching
    pattern_constructor_name = ~excluded_keywords + \
        constructor_identifier.copy()  # Constructor names in patterns
    ...
    record_literal = (
        ...
        Suppress("}")
        # Primary expressions - try tuple first, then parenthesized (to handle both cases)
    ).setParseAction(lambda t: ("RECORD", list(t)))
    primary_expr = record_literal | tuple_expr | list_literal | atom

    # Patterns (simplified)
    pattern_var = pattern_identifier.setParseAction(
        lambda t: ("PATTERN_VAR", t[0]))
    pattern_wildcard = placeholder.setParseAction(
        lambda t: ("PATTERN_WILDCARD", "_"))
    pattern_literal = (string_literal | char_literal | number).setParseAction(
        lambda t: ("PATTERN_LITERAL", t[0]))
    ...
    pattern_constructor = (
        Suppress("(") + pattern_constructor_name +
        ZeroOrMore(pattern) + Suppress(")")
    ...
    function_name = (math_ops | binding_identifier).setParseAction(
        lambda t: ("FUNCTION_NAME", t[0]))
    function_call = Group(
    ...
    lambda_expr = (
        lambda_kw + OneOrMore(pattern_identifier) +
        Suppress(double_arrow_op) + expression
    ...
    # Parallel operations
    par_map = Keyword(
        "par-map").setParseAction(lambda t: ("IDENTIFIER", "par-map"))
    par_fold = Keyword(
        "par-fold").setParseAction(lambda t: ("IDENTIFIER", "par-fold"))
    par_filter = Keyword(
        "par-filter").setParseAction(lambda t: ("IDENTIFIER", "par-filter"))
    ...
    dollar_expr <<= (infix_binding_expr + PyParsingOptional(
        Suppress(dollar_op) + dollar_expr)).setParseAction(make_dollar)
    ...
    def parse_semicolon_expr(tokens):
      ...
      for i in range(len(tokens) - 2, -1, -1):
        ...
        if (isinstance(expr, tuple) and len(expr) == 2 and
          ...
          # Extract variable name from ("IDENTIFIER", "x")
          var_name = expr[1][1][1]
          var_value = expr[1][2] if len(
              expr[1]) > 2 else ("ERROR", "missing_value")
    ...
    pipeline_expr = semicolon_expr + \
        ZeroOrMore(Suppress(pipeline_op) + semicolon_expr)
    ...
    # Contract expressions - use a simplified expression grammar to avoid circular dependency
    expression <<= pipeline_expr
    # Contract expressions can be basic comparisons, function calls, and simple operations
    contract_identifier = binding_identifier.setParseAction(
        lambda t: ("IDENTIFIER", t[0]))

    # Allow operators as function names in contracts (for prefix notation like (/= x y))
    contract_operator = oneOf(
        "/= != == <= >= < > + - * / % mod").setParseAction(lambda t: ("FUNCTION_NAME", t[0]))
    ...
    contract_primary = (
        ...
        number |
        contract_identifier |
        # Forward declaration for nested expressions
        Suppress("(") + Forward() + Suppress(")")
    )
    ...
    type_identifier = Regex(
        r'[a-zA-Z_][a-zA-Z0-9_\'-]*(?:\.[a-zA-Z_][a-zA-Z0-9_\'-]*)*')
    type_var_pattern = Regex(r'[a-z][a-zA-Z0-9_]*')

    # Reuse the same excluded_keywords from above
    type_atom = (~excluded_keywords +
                 type_identifier).setParseAction(lambda t: ("TYPE_ATOM", t[0]))
    type_var = (~excluded_keywords +
                type_var_pattern).setParseAction(lambda t: ("TYPE_VAR", t[0]))
    ...
    type_paren_app = (
        Suppress("(") + type_atom +
        OneOrMore(type_atom | type_var) + Suppress(")")
    ...
    def parse_function_signature_type(tokens):
      ...
      if len(result_tokens) == 1:
        ...
        if has_arrow:
          ...
          # Mark as error/invalid
          result_type = ("TYPE_SEQUENCE", result_tokens)
    ...
    typed_param = (
        Suppress("(") + pattern_identifier +
        Suppress(colon_op) + type_expr + Suppress(")")
    ...
    untyped_param = pattern.copy().setParseAction(
        lambda t: ("UNTYPED_PARAM", t[0]))
    ...
    return_type_named = (
        Suppress("->") + Suppress("(") + pattern_identifier +
        Suppress(colon_op) + type_expr + Suppress(")")
