...
def try_match_clause(args: List[Dict], clause: Dict, env: Dict, debug: bool = False, context: Optional[Dict] = None) -> Optional[Dict]:
    ...
    for i, (arg_val, pattern) in enumerate(zip(args, patterns)):
        ...
        if debug:
            ...
            print(f"  Arg {i}: type={arg_val.get('type')}, value={arg_val.get('value')}")
        ...
        if type_constraint:
            ...
            expected_type = type_constraint.get('name', type_constraint) if isinstance(type_constraint, dict) else type_constraint
            ...
            if expected_type == 'List':
                ...
                if debug:
                    print(f"  Type mismatch: {expected_type} required but got {arg_type}")
                return None
...
def io_read(filename: Dict, context: Dict) -> str:
    ...
    try:
        fname = filename['value'] if isinstance(filename, dict) and 'value' in filename else filename
...
def io_write(filename: Dict, content: Dict, context: Dict) -> None:
    ...
    try:
        fname = filename['value'] if isinstance(filename, dict) and 'value' in filename else filename
        cont = content['value'] if isinstance(content, dict) and 'value' in content else content
    ...
    def on_receive(self, message):
        ...
        try:
            ...
            if self.handler_func['type'] == 'function':
                ...
                if len(params) >= 1:
                    call_env = env_bind_value(self.handler_func['closure_env'], params[0], msg_val)
...
def validate_contract(contract_ast: Dict, param_names: List[str], arg_values: List[Dict],
    ...
    try:
        ...
        if condition_result['type'] != 'Bool':
            raise REI1RuntimeError(f"Contract condition must be boolean, got {condition_result['type']}")

        if not condition_result['value']:
            condition_type = "Precondition" if is_precondition else "Postcondition"
            # Build helpful error message
            if is_precondition:
                args_str = ', '.join(f"{name}={val['value']}" for name, val in zip(param_names, arg_values))
                raise REI1RuntimeError(f"{condition_type} violated with arguments: {args_str}")
            else:
                result_str = result_value['value'] if result_value else 'None'
                raise REI1RuntimeError(f"{condition_type} violated: result={result_str}")
...
def rei1_Type(constructor_ast: Dict) -> Dict:
    ...
    if constructor_ast.get('type') != 'FUNCTION_CALL':
        raise RuntimeError(f"Type: expected FUNCTION_CALL, got {constructor_ast.get('type')}")
...
def eval_ast(ast_node: Dict, env: Dict, debug: bool = False, context: Optional[Dict] = None) -> Tuple[Dict, Dict]:
    ...
    if node_type == "NUMBER":
        ...
        func_name = context.get('current_function', '<unknown>') if context else '<unknown>'
        raise REI1RuntimeError(f"Function '{func_name}' is not yet implemented (marked with ??)")
    ...
    if func_val['type'] == 'BuiltinOperator':
        ...
        if op_name in BUILTIN_OPERATORS:
            ...
            if len(args) >= 2:
                ...
                raise RuntimeError(f"Operator {op_name} requires 2 arguments, got {len(args)}")
        else:
        ...
        if receives_ast_flag:
            if len(args_ast) != arity:
                raise RuntimeError(f"Function {func_data['name']} expects {arity} arguments, got {len(args_ast)}")
            ...
            try:
                if needs_context_flag:
                    ...
                    if arity == 1:
                        ...
                        raise RuntimeError(f"Unsupported arity for receives_ast+needs_context: {arity}")
                else:
        ...
        if len(args) < arity:
            ...
            raise RuntimeError(f"Function {func_data['name']} expects {arity} arguments, got {len(args)}")
        ...
        if len(args) != arity:
            raise RuntimeError(f"Constructor {ctor_name} expects {arity} arguments, got {len(args)}")
        ...
        if len(args) != 1:
            raise RuntimeError(f"Signature expects 1 argument (implementation record), got {len(args)}")
        ...
        if len(args) < len(func_params):
            ...
            raise RuntimeError(f"Function expects {len(func_params)} arguments, got {len(args)}")

        # Check precondition if contract exists
        if func_contract:
            validate_contract(func_contract, func_params, args, None, func_env, True, debug)
        ...
        if func_contract:
            validate_contract(func_contract, func_params, args, result_val, func_env, False, debug)
        ...
        if debug:
            print(f"DEBUG: multi_clause_function '{func_name}' has {len(clauses)} clauses")
        ...
        for i, clause in enumerate(clauses):
            ...
            if bindings is not None:
                ...
                if clause_contract:
                    ...
                    validate_contract(clause_contract, param_names, param_values, None, func_env, True, debug)
                ...
                if clause_contract:
                    param_names = list(bindings.keys())
                    param_values = [bindings[name] for name in param_names]
                    validate_contract(clause_contract, param_names, param_values, result_val, func_env, False, debug)
        ...
        if partial_data.get('type') == 'partial_builtin':
            ...
            if len(complete_args) < total_arity:
                ...
                raise RuntimeError(f"Too many arguments for partial function: expected {total_arity}, got {len(complete_args)}")
            ...
            try:
                if needs_context_flag:
                    if total_arity == 1:
                        ...
                        result = func_impl(complete_args[0], complete_args[1], complete_args[2], context)
                    else:
                    ...
                    if total_arity == 1:
                        ...
                        result = func_impl(complete_args[0], complete_args[1], complete_args[2], env)
                    else:
                        raise RuntimeError(f"Unsupported arity: {total_arity}")
                else:
                    if total_arity == 1:
                        ...
                        result = func_impl(complete_args[0], complete_args[1], complete_args[2])
                    else:
                        raise RuntimeError(f"Unsupported arity: {total_arity}")
                ...
                raise RuntimeError(f"Error calling partial builtin {func_data['name']}: {e}")
            ...
            if len(complete_args) < len(func_params):
                ...
                raise RuntimeError(f"Too many arguments for partial function: expected {len(func_params)}, got {len(complete_args)}")
            ...
            if len(args) != len(placeholder_positions):
                raise RuntimeError(f"Partial function expects {len(placeholder_positions)} arguments, got {len(args)}")
            ...
            if original_func['type'] == 'BuiltinOperator':
                ...
                if op_name in BUILTIN_OPERATORS:
                    ...
                    if len(complete_args) >= 2:
                        ...
                        raise RuntimeError(f"Operator {op_name} requires 2 arguments, got {len(complete_args)}")
                else:
                    raise RuntimeError(f"Unknown operator: {op_name}")
                ...
                if len(complete_args) != total_arity:
                    raise RuntimeError(f"Function {func_data['name']} expects {total_arity} arguments, got {len(complete_args)}")
                ...
                if len(complete_args) != len(func_params):
                    raise RuntimeError(f"Function expects {len(func_params)} arguments, got {len(complete_args)}")
                ...
                raise RuntimeError(f"Cannot apply partial function to {original_func['type']}")
        else:
            ...
            if isinstance(param_tuple, tuple) and len(param_tuple) >= 2:
                ...
                if param_type == "TYPED_PARAM" and isinstance(param_data, dict):
                    ...
                    if isinstance(name_tuple, tuple) and len(name_tuple) >= 2:
                        ...
                        if debug:
                            print(f"DEBUG: Extracted type_name={type_name} from constraint={type_constraint}")
    ...
    if existing_func and param_patterns:  # Has patterns, so could be multi-clause
        ...
        if existing_func.get('type') == 'function':
            ...
            first_clause = make_function_clause(
                [{'type': 'PATTERN_VAR', 'value': p} for p in existing_func['params']],
                existing_func['body'],
                None,  # No guard in old-style function
                existing_func.get('contract')
            )
            new_clause = make_function_clause(param_patterns, func_body, guard_ast, contract_ast)
            ...
            if param_names:
                ...
                clause = make_function_clause(param_patterns, func_body, guard_ast, contract_ast)
        ...
        has_type_constraints = any(p.get('type_constraint') for p in param_patterns)
    ...
    for branch in branches:
        if branch['type'] == 'CASE_BRANCH':
            ...
            if match_result is not None:
                ...
                pattern_bindings = match_result if isinstance(match_result, dict) else {}
                branch_env = env
    ...
    if pattern_ast['type'] == 'PATTERN_LITERAL':
        ...
        if pattern_ctor_name == 'cons':
            ...
            if isinstance(head_pattern, tuple) and head_pattern[0] == 'PATTERN_VAR':
                ...
                # Try recursive match
                head_result = matches_pattern(head_val, {'type': head_pattern[0], 'value': head_pattern[1]} if isinstance(head_pattern, tuple) else head_pattern, env)
                if head_result is None:
                    return None
                all_bindings.update(head_result)
            ...
            if isinstance(tail_pattern, tuple) and tail_pattern[0] == 'PATTERN_VAR':
                ...
                # Try recursive match
                tail_result = matches_pattern(tail_val, {'type': tail_pattern[0], 'value': tail_pattern[1]} if isinstance(tail_pattern, tuple) else tail_pattern, env)
                if tail_result is None:
                    return None
                all_bindings.update(tail_result)
...
def create_interpreter(debug: bool = False):
    ...
    def eval_node_wrapper(ast_node, env_obj):
        ...
        actual_env = env_obj._dict if hasattr(env_obj, '_dict') else create_builtin_runtime_env()
        # Convert ast_node back to dict if it's a wrapper
        actual_ast = {'type': ast_node.type, 'value': ast_node.value} if hasattr(ast_node, 'type') else ast_node
