...
def analyze_function_call_tuple(call_data, env: Dict, debug: bool = False) -> Dict:
    ...
    if isinstance(call_data, dict):
        if 'name' in call_data and 'args' in call_data:
            ...
            arg_asts = [analyze_expression(arg, env, debug) for arg in call_data['args']]
        else:
...
def analyze_case_tuple(case_data, env: Dict, debug: bool = False) -> Dict:
    ...
    for branch in branches:
        if isinstance(branch, tuple) and len(branch) >= 2:
            ...
            if branch_type in ["CASE_BRANCH", "CASE_ARM"] and isinstance(branch_value, dict):
                ...
                pattern_ast = analyze_expression(pattern, branch_env, debug) if pattern else None
                body_ast = analyze_expression(body, branch_env, debug) if body else None
    ...
    result_type = branch_asts[0]['type_info'] if branch_asts else make_type_info("Unknown")
...
def analyze_list_tuple(list_data, env: Dict, debug: bool = False) -> Dict:
    ...
    elem_type = element_asts[0]['type_info'] if element_asts else make_type_info("Unknown")
...
def analyze_contract(contract_data, env: Dict, return_type: Optional[Dict] = None, debug: bool = False) -> Dict:
    ...
    if contract_value.get("post"):
        post_env = env_bind(env, "result", return_type or make_type_info("Unknown"))
...
def analyze_cst_node(cst_node: CSTNode, env: Dict, debug: bool = False) -> Dict:
    ...
    if cst_node.type in handlers:
        ...
        children = [analyze_cst_node(child, env, debug) for child in cst_node.children]
...
def analyze_function_def(cst_node: CSTNode, env: Dict, debug: bool = False) -> Dict:
    ...
    if func_data.get('contract'):
        contract_ast = analyze_contract(func_data['contract'], func_env, return_type, debug)
    ...
    func_type = make_type_info("Function", param_types + ([return_type] if return_type else []))
    ...
    return make_ast_node(
        "FUNCTION_DEF",
        {
            "name": func_name,
            "params": param_types,
            "raw_params": params_for_interpreter,  # Raw params for interpreter pattern matching
...
def analyze_value_binding(cst_node: CSTNode, env: Dict, debug: bool = False) -> Dict:
    ...
    value_type = value_ast['type_info'] if value_ast else make_type_info("Unknown")
...
def analyze_identifier(cst_node: CSTNode, env: Dict, debug: bool = False) -> Dict:
    """Analyze identifier"""
    name = cst_node.value if isinstance(cst_node.value, str) else str(cst_node.value)
...
def analyze_function_call(cst_node: CSTNode, env: Dict, debug: bool = False) -> Dict:
    ...
    if isinstance(cst_node.value, list):
        ...
        if len(cst_node.children) >= 1:
            ...
            arg_asts = [analyze_cst_node(child, env, debug) for child in cst_node.children[1:]]
...
def analyze_list(cst_node: CSTNode, env: Dict, debug: bool = False) -> Dict:
    ...
    children = [analyze_cst_node(child, env, debug) for child in cst_node.children]
    elem_type = children[0]['type_info'] if children else make_type_info("Unknown")
    list_type = make_type_info("List", [elem_type])
